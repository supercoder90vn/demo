...
app.factory('Task', function(FURL, $firebase, Auth) {
	...
	var user = Auth.user;
	tasks.$loaded().then(function(tasks){
	...
	var Task = {
		...
		createTask: function(task) {
			...
			return tasks.$add(task).then(function(newTask) {
				...
				// option 1 return $firebase(ref.child('user_tasks').child(task.poster)).$asArray().$add(obj);
				return $firebase(ref.child('user_tasks').child(task.poster)).$push(obj);
			});
		},

		createUserTasks: function(taskId) {//~~~~~~~~~~~~~~~~~~ PHUC
			var taskObj = Task.getTask(taskId).$asObject();
			taskObj.$loaded()
				.then(function(task) {
					console.log("__task.js_createUserTasks")
					console.log("$asObject() === task" + (taskObj === task));
					...
					// option 1: return $firebase(ref.child('user_tasks').child(task.runner)).$asArray().$add(obj);	
					// option 2
